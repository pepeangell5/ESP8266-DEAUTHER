    ...
    createMenu(&apListMenu, &showMenu, [this]() {
        ...
        for (int i = 0; i < c; i++) {
            addMenuNode(&apListMenu, [i]() {
                ...
                selectedID = i;
                if (!accesspoints.getSelected(i)) accesspoints.select(i);
                changeMenu(&apMenu);
            });
        }
...
void DisplayUI::drawMenu() {
    String tmp;
    int    tmpLen;
    // Bajamos a 4 filas para que respiren y no se corten
    int    row = (currentMenu->selected / 4) * 4; 

    // --- 1. ENCABEZADO CORREGIDO ---
    display.setColor(WHITE);
    display.fillRect(0, 0, 128, 13); // Barra superior un poco más alta
    display.setColor(BLACK);
    
    // Usamos la fuente ArialMT_Plain_10 que es más compacta para el título
    display.setFont(ArialMT_Plain_10);
    
    String title = "";
    if(currentMenu == &mainMenu) title = "PEPEANGELL - OS";
    else if(currentMenu == &scanMenu) title = "ESCANEAR";
    else if(currentMenu == &attackMenu) title = "ATAQUES";
    else if(currentMenu == &showMenu) title = "RESULTADOS";
    else title = "MENU";
    
    // Centrado manual: 64 es la mitad de 128. TEXT_ALIGN_CENTER hace la magia.
    display.setTextAlignment(TEXT_ALIGN_CENTER);
    display.drawString(64, 0, title);
    display.setTextAlignment(TEXT_ALIGN_LEFT); // Reset para el resto del menú

    // Regresamos a blanco para el contenido
    display.setColor(WHITE);

    // Corregir selección si se sale de rango
    ...
    // --- 2. DIBUJAR ENTRADAS (Letra más pequeña para que no se corte) ---
    // Usamos ArialMT_Plain_10 también para la lista, es más elegante y no se corta
    display.setFont(ArialMT_Plain_10);
    ...
    for (int i = row; i < currentMenu->list->size() && i < row + 4; i++) {
        ...
        // Calculamos la posición Y dejando espacio para el encabezado
        int yPos = 16 + ((i - row) * 12); 

        if (currentMenu->selected == i) {
            // Dibujamos la barra de selección (un poco más ancha que el texto)
            display.setColor(WHITE);
            display.fillRect(0, yPos - 1, 124, 11); 
            display.setColor(BLACK); // Texto negro sobre barra blanca
            
            // Si el texto es muy largo, aplicamos el scroll horizontal
            if (tmpLen >= 18) { // 18 caracteres es el límite aprox para esta fuente
                tmp = tmp + "   " + tmp;
                tmp = tmp.substring(scrollCounter, scrollCounter + 18);

                if (((scrollCounter > 0) && (scrollTime < currentTime - scrollSpeed)) || ((scrollCounter == 0) && (scrollTime < currentTime - scrollSpeed * 4))) {
                    scrollTime = currentTime;
                    scrollCounter++;
                }
                if (scrollCounter > tmpLen + 3) scrollCounter = 0;
            }
        } else {
            display.setColor(WHITE);
        }

        // Dibujamos el texto con un pequeño margen a la izquierda
        display.drawString(4, yPos - 1, tmp);
    }
    
    // --- 3. INDICADOR DE SCROLL (Barra lateral derecha) ---
    if(currentMenu->list->size() > 4) {
        int barAreaHeight = 64 - 16;
        int barHeight = barAreaHeight / currentMenu->list->size();
        if (barHeight < 4) barHeight = 4; // Tamaño mínimo para que se vea
        int barY = 16 + (currentMenu->selected * (barAreaHeight - barHeight) / (currentMenu->list->size() - 1));
        
        display.setColor(WHITE);
        display.drawVerticalLine(127, 16, 48); // Línea guía
        display.fillRect(125, barY, 2, barHeight); // Indicador gordo
    }
}
